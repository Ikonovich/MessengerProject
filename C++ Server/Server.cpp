...
class Server {
    ...
    // Database variables.
        ...
        // IR Return packet format:
        // "UserID": [User ID - 16 chars - An ID that should be all zeros.]
        // "Data": [User name - At least 6 and up to 16 chars - a username terminated with %]

        // IR Return packet format:
        // [Response type - 2 chars]
        // [User ID - 16 chars - A new ID assigned to the user.]
        //
        void LoginRequest(std::string input, int clientSocket) {
            ...
            ParseUser(input, senderID, remainder);
        ...
        void MessageReceived(std::string input, int clientSocket) {
        ...
        void InitialRegistration(std::string input, int clientSocket) 
        {

            // // std::cout << "Registration request made.\n";

            // // std::string userID = input["UserID"];
            // // int nameEnd = input["Data"].find("%");
            // // std::string userName = input["Data"].substr(0, nameEnd);

            // // if (nameEnd > 15) {

            // //     TransmissionHandler(clientSocket, "AM The provided username was too long.\n");
            // //     return;
            // // }
            // // else if (nameEnd < 5) {

            // //     TransmissionHandler(clientSocket, "AM The provided username was too short.\n");
            // //     return;
            // // }
            // // else if (NameToIDMap.count(userName) > 0) {

            // //     TransmissionHandler(clientSocket, "AM The provided username was already in use.\n");
            // //     return;
            // }

            // if (userID != "0000000000000000") {
            //     std::cout << "Error: A non-zero hash " << userID << " has been received during a registration request.\n";
            //     TransmissionHandler(clientSocket, "Error: A non-zero hash has been received during a registration request.\n");
            // }
            ...
            //     int nextCharInt = rand() % 26 + 65;
            //     char nextChar = nextCharInt;
            //     newUserID.push_back(nextChar);
            // }

            // std::cout << "New user ID: " << newUserID << "\n";

            // SocketMap[newUserID] = clientSocket;
            
            // NameToIDMap[userName] = newUserID;
            // IDtoNameMap[newUserID] = userName;

            // std::string response = "RS" + newUserID + "Registration for username " + userName + " has been successful.\n";
            //     response += " Your new UserID is: " + newUserID + "\n";
            // TransmissionHandler(newUserID, response);
        ...
        int TransmissionHandler(std::string userID, std::string response) {
        ...
        int TransmissionHandler(int responseSocket, std::string response) {
        ...
        void ConnectDatabase() {
            ...
            result = statement->executeQuery("SELECT * FROM RegisteredUsers WHERE UserName=\"Ikonovich\"");
    

            while (result->next()) {

                std::cout << "MySQL results: ";
